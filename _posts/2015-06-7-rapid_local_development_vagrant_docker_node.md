...
I also wanted to do development on a custom module. Usually on your local machine you would do `npm link` for rapid iteration on a custom module. However working with Vagrant and Docker makes this more complicated.
...
### What about that commited symlink?
So finally I'll address the commited symlink.

`node_modules -> /dist/node_modules/`

When production with the default container Dockerfile  and without any `-v` volumes. It creates
`/dist/node_modules` and manually creates an internal symlink from our application code there: `RUN ln -s /dist/node_modules /srv/www/node_modules`

When running the container in Vagrant and mounting the application code as a volume with `-v '/vagrant/web:/srv/www'` 
